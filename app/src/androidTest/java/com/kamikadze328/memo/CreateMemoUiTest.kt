...
import com.kamikadze328.memo.view.create.location.ChooseLocation
import com.kamikadze328.memo.view.create.location.ChooseLocationContract
import com.kamikadze328.memo.view.create.location.ChooseLocationResult
...
class CreateMemoUiTest {
    ...
    // Initialize Espresso Intents once BEFORE launching the Activity
    @get:Rule(order = 0)
    ...
    fun save_withEmptyFields_showsErrors() {
        ActivityScenario.launch(CreateMemo::class.java).use {
            // Click Save
            ...
            onView(withId(R.id.choose_location_text))
                .check(matches(withText(getString(R.string.memo_location_empty_error))))
        }
    }
    ...
    fun save_afterChoosingLocation_finishesWithOk() {
        ...
        Intents.intending(hasComponent(ChooseLocation::class.java.name))
            .respondWith(
        ...
        val intent: Intent = Intent(context, CreateMemo::class.java)
        val scenario = ActivityScenario.launchActivityForResult<CreateMemo>(intent)
